...
This skill is the orchestration layer over the `nl-gov-data` MCP. The MCP provides the normalized data surface; this skill turns that surface into workflow-ready public-affairs outputs. Design for the hosted remote FastMCP server as the normal runtime; use a local stdio checkout only as a development or validation fallback when it is available.
...
Prefer the configured remote MCP endpoint for normal use. The intended portable deployment is the hosted FastMCP server, so do not assume a local `MCPs/nl-gov-data` checkout exists on the user's machine. If the remote endpoint returns `401 Unauthorized`, treat that as an authentication/configuration issue and report it clearly; do not silently switch to local data unless the user is working in this repo and local validation is explicitly acceptable.

Local stdio is useful for testing this skill in ClaudeCowork because this repo may sit next to `../MCPs/nl-gov-data`, but it is not the production assumption. When using local stdio, record that in `metadata.assumptions` or the final note so downstream users know the result was not fetched through the hosted endpoint.
...
Use the bundled validation harness when you need to verify that the workflow templates still work against the live MCP:

```bash
uv run python .claude/skills/dutch-gov-data/scripts/validate_workflows.py --workflow all
```

The validator connects to the local `MCPs/nl-gov-data` stdio server and writes structured workflow outputs when requested. Read the script itself when adapting or debugging the flow.
